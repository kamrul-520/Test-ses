...
def old_clone():
    ...
    linex()
    # ↓↓↓ নতুন অপশন (SAFE)
    print('       \x1b[38;5;196m(\x1b[1;37mD\x1b[38;5;196m)\x1b[1;37m\x1b[38;5;49m2015–2017 UID TOOLS (SAFE)')
    linex()
...
def old_clone():
    ...
    if _input in ('A','a','01','1'):
        ...
        old_Tree()
    elif _input in ('D','d','04','4'):  # ← নতুন রুট
        old_Four_safe()
    else:
        print(f"\n[×]{rad} Choose Value Option... ")
        BNG_71_()
...
def old_Four_safe():
    ...
    user = []
    ____banner____()
    print(f"       2015–2017 UID TOOLS (SAFE)")
    linex()
    print(f"       Example {Y}:{G} 2000 / 5000 / 10000")
    limit = input(f"       TOTAL UIDs TO GENERATE {Y}:{G} ")
    ...
    prefixes = ['100009', '10001', '10002']  # 2015, 2016, 2017
    ...
    ____banner____()
    print('       (A) Validate & Summarize')
    print('       (B) Export to file')
    print('       (C) Preview first 5000')
    linex()
    meth = input(f"CHOICE (A/B/C): {Y}").strip().upper()

    if meth == 'A':
        summarize_2015_2017(user)
    elif meth == 'B':
        path = '/sdcard/KAMRUL-UIDS-2015-2017.txt'
        try:
            with open(path, 'w') as f:
                for u in user:
                    f.write(u + '\n')
            print(f"{G}✔ Exported {len(user)} UIDs to: {path}{W}")
        except Exception as e:
            print(f"{rad}× Failed to write file: {e}{W}")
    elif meth == 'C':
        print(f"{G}Showing first 50 UIDs:{W}")
        for u in user[:50]:
            print(f"{u}  ~ year:{creationyear_safe(u)}")
    else:
...
def summarize_2015_2017(uid_list):
    c15 = sum(1 for u in uid_list if u.startswith('100009'))
    c16 = sum(1 for u in uid_list if u.startswith('10001'))
    c17 = sum(1 for u in uid_list if u.startswith('10002'))
    total = len(uid_list)
    print(f"\n{G}Summary (SAFE){W}")
    linex()
    print(f"2015 (100009): {c15}")
    print(f"2016 (10001) : {c16}")
    print(f"2017 (10002) : {c17}")
    linex()
